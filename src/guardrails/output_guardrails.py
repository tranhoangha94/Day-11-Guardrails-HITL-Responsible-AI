...
def content_filter(response: str) -> dict:
    ...
    PII_PATTERNS = {
        ...
        "known_secret": r"admin123|sk-vinbank-secret",
        "internal_host": r"db\.[a-z0-9.-]+\.internal(?::\d+)?",
    }
...
_init_judge_called = False


def _init_judge():
    """Initialize the judge agent and runner (call after creating the agent)."""
    global judge_runner, _init_judge_called
    if safety_judge_agent is not None and not _init_judge_called:
        judge_runner = runners.InMemoryRunner(
            agent=safety_judge_agent, app_name="safety_judge"
        )
        _init_judge_called = True
